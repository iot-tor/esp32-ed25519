...
struct f25519_t {
  ...
  f25519_t(const f25519_t &a) {
    mpi=new mbedtls_mpi();
    mbedtls_mpi_init(mpi);
    mbedtls_mpi_copy(mpi,a.mpi);
  }

  void operator=(const f25519_t &a) {
    mbedtls_mpi_copy(mpi,a.mpi);
  }
  ...
  void set(int o) {
    ...
    unsigned char q;
    q=o;
    mbedtls_mpi_read_binary(mpi,&q,1);
  }
  ...
  void set25519() {
    ...
    memset(q,0,32);
    q[0]=0x80;
    mbedtls_mpi_read_binary(mpi,q,32);
    mbedtls_mpi_sub_int(mpi,mpi,19);
  }
  ...
  void aff() const {
    unsigned char q[32];
    mbedtls_mpi_write_binary(mpi,q,32);
    for(int i=31;i>=0;i--)
      printf("%02x ",q[i]);
    printf("%d ",mbedtls_mpi_cmp_int(mpi,0));
    printf("\n");
  }
  ...
  unsigned char parity() const {
    return mbedtls_mpi_get_bit(mpi,0);
  }
};

f25519_t p25519;

void f25519_t::reduce() {
  mbedtls_mpi_mod_mpi(mpi,mpi,p25519.mpi);
}
...
{
  return mbedtls_mpi_cmp_mpi(x.mpi,y.mpi)==0;
}
...
{
  mbedtls_mpi_mul_int(r.mpi,a.mpi,b);
  r.reduce();
}
		       ...
		       const uint8_t *modulus)
{
  ...
  mbedtls_mpi_read_binary(&bnx,r,64);
  for(int u=0;u<32;u++)
    r[u]=modulus[31-u];
  mbedtls_mpi bnm;
  mbedtls_mpi_init(&bnm);
  mbedtls_mpi_read_binary(&bnm,r,32);
	
  mbedtls_mpi_mod_mpi(&bnx,&bnx,&bnm);
  mbedtls_mpi_write_binary(&bnx,r,32);
  for(int u=0;u<32;u++)
    n[u]=r[31-u];
